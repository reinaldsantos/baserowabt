...
Este documento descreve a implementação de uma base de dados Baserow instalada localmente via Docker, configurada para funcionar em rede local e utilizada para gerenciar uma clínica médica fictícia. O projeto atende aos seguintes requisitos:
...
| IP da Máquina | 192.168.0.109 (pode variar – verificar com ipconfig) |
| Porta exposta | 8080 |
...
Baixar do site oficial e seguir o instalador.

Ativar a virtualização na BIOS/UEFI

Reiniciar e acessar a BIOS (tecla comum: F2, F10, Del).
...
O container foi criado com o comando:
...
- `-v baserow_data:/baserow/data` → volume persistente para manter os dados.
...
Tabela | Quantidade | IDs exemplo
---|---|---
Pacientes | 5 | 6, 7, 8, 9, 10
Médicos | 3 | 3, 4, 5
Consultas | 3 | 4, 5, 6
...
6.1 Database Token (para CRUD)

O token utilizado para operações nas linhas das tabelas é:

```text
<<SEU_DATABASE_TOKEN_AQUI>>
```

Ele deve ser enviado no cabeçalho de todas as requisições:
...
Obtido via autenticação com e‑mail e senha. Para gerar, execute:
...
A resposta conterá os campos `access_token` e `token`. Esse token deve ser usado no cabeçalho `Authorization: JWT <token>` para operações de estrutura (criar campos, tabelas, etc.).
...
Todas as requisições devem incluir o cabeçalho de autenticação com o Database Token.
...
```bash
curl -X GET -H "Authorization: Token <<SEU_DATABASE_TOKEN_AQUI>>" \
...
```bash
curl -X POST -H "Authorization: Token <<SEU_DATABASE_TOKEN_AQUI>>" \
...
```bash
curl -X PATCH -H "Authorization: Token <<SEU_DATABASE_TOKEN_AQUI>>" \
  -H "Content-Type: application/json" \
  -d "{\"Number\":\"99999\"}" \
...
```bash
curl -X DELETE -H "Authorization: Token <<SEU_DATABASE_TOKEN_AQUI>>" \
...
Após acessar, faça login com as credenciais cadastradas:
...
8.1 Listar todos os médicos
bash
curl -X GET -H "Authorization: Token ZflqGRqnGhdjsYEZ4gzZVP0wH9e1J2KN" \
  "http://192.168.0.109:8080/api/database/rows/table/713/?user_field_names=true"
8.2 Criar uma nova consulta
bash
curl -X POST -H "Authorization: Token ZflqGRqnGhdjsYEZ4gzZVP0wH9e1J2KN" \
  -H "Content-Type: application/json" \
  -d "{\"DataConsulta\":\"2024-03-25\",\"HoraConsulta\":\"14:30\",\"Status\":\"Agendada\",\"Notes\":\"Consulta de rotina\",\"Paciente\":[6],\"Medico\":[3]}" \
  "http://192.168.0.109:8080/api/database/rows/table/712/?user_field_names=true"
8.3 Atualizar o telefone de um paciente (campo Number)
bash
curl -X PATCH -H "Authorization: Token ZflqGRqnGhdjsYEZ4gzZVP0wH9e1J2KN" \
  -H "Content-Type: application/json" \
  -d "{\"Number\":\"99999\"}" \
  "http://192.168.0.109:8080/api/database/rows/table/711/6/?user_field_names=true"
8.4 Deletar um médico
bash
curl -X DELETE -H "Authorization: Token ZflqGRqnGhdjsYEZ4gzZVP0wH9e1J2KN" \
  "http://192.168.0.109:8080/api/database/rows/table/713/6/?user_field_names=true"
9. Acessando a Interface Web
Acesso local: http://localhost:8080 ou http://192.168.0.109:8080
...
docker run -d ... -e BASEROW_PUBLIC_URL=http://novo_ip:8080 ...
12. Estado Atual (23/03/2026)
Container: rodando, saudável.
...
Autor: Gabriel Santos
Data: 23/03/2026#   b a s e r o w a b t 
 
 #   b a s e r o w a b t  
 